...
Public Class Form1
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        plot11.ChartType() = SeriesChartType.Spline
        ...
        For j As Integer = -R To R
            Y = j
            X = Math.Sqrt(R * R - Y * Y)
            plot11.Points.AddXY(X, Y)

        Next


        'верхния наклонная (1 семестр)
        For j As Integer = 0 To R
            Y = j
            X = R - Y
            plot2.Points.AddXY(X, Y)
        Next
        'верхния наклонная (2 семестр)
        For j As Integer = 0 To R
            Y = j
            X = R - Y
            plot2.Points.AddXY(-X, Y)
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        ...
        Try
            ...
            For i As Integer = 1 To n
                ...
                '         circle                 1 semestr      2 semestr         3 semestr        4 semestr                                                                         top line               bot line
                If ((R * R - x * x >= y * y) And ((x > R - y) Or (-x > R - y) Or (-x >= y + R) Or (y <= x - R))) Then
